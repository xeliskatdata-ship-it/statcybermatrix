...
-- Corpus normalise pour le matching
articles_base AS (
    SELECT
        id,
        source,
...
-- Jointure keyword x article avec comptage d'occurrences par document
matched AS (
    SELECT
        k.keyword,
        k.category,
    ...
    FROM kw_list k
    JOIN articles_base a
        ON a.corpus LIKE '%' || k.keyword || '%'
)

-- Agregation par mot-cle x periode glissante
SELECT
    k.keyword,
    k.category,
    k.sub_category,
    ...
    -- Nombre de sources distinctes ayant couvert ce mot-cle
    COUNT(
        DISTINCT CASE
            WHEN m.published_date >= CURRENT_DATE - (p.period_days || ' days')::INTERVAL
            THEN m.source
