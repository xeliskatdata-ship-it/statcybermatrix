...
def translate_to_french(text):
    """
    Traduit un texte anglais en francais via Google Translate (deep-translator).
    ...
    try:
        ...
        return text  # Fallback : texte original si traduction echoue


@st.cache_data(ttl=3600)  # Cache 1 heure pour ne pas surcharger l'API
def fetch_cve_details(cve_id, lang='fr'):
    """
    Interroge l'API NVD (National Vulnerability Database) pour obtenir
    les details officiels d'une CVE.

    API utilisee : https://services.nvd.nist.gov/rest/json/cves/2.0
    Gratuite, sans cle API, limite a 5 requetes / 30 secondes.

    Retourne un dictionnaire avec :
      - description   : texte officiel de la vulnerabilite
      - score         : score CVSS v3 (0.0 a 10.0)
      - severity      : CRITICAL / HIGH / MEDIUM / LOW / NONE
      - produit       : nom du logiciel / systeme affecte
      - published     : date de publication
      - references    : liens officiels
    """
    ...
    try:
        ...
        # Score CVSS v3 (ou v2 en fallback)
        score    = None
        severity = 'N/A'
        ...
        if published != 'N/A':
            published = published[:10]  # Garder seulement YYYY-MM-DD

        # Traduire la description si langue francaise demandee
...
def get_score_class(severity):
    """Retourne la classe CSS selon la severite."""
    mapping = {
...
def render_cve_card(details):
    """Affiche une fiche complete pour une CVE."""
    if not details:
...
df['cves'] = df.apply(extract_cves, axis=1)
df_cve     = df.explode('cves').dropna(subset=['cves'])
df_cve     = df_cve[df_cve['cves'].str.strip() != '']
df_cve['cves'] = df_cve['cves'].str.upper()
...
# --- Agregation ---
top_n = st.slider("Nombre de CVE a afficher", 5, 20, 10, key="k6_topn")
agg   = df_cve['cves'].value_counts().head(top_n).reset_index()
...
# --- Insights ---
total_uniques     = df_cve['cves'].nunique()
articles_avec_cve = df_cve['url'].nunique()
...
<div class="insight-box">
    ...
    - Total de CVE uniques detectees : <b>{total_uniques}</b>.<br>
    - Articles contenant au moins une CVE : <b>{articles_avec_cve}</b> / {len(df)}.
</div>
""", unsafe_allow_html=True)
...
if st.button("Charger les details depuis NVD", key="k6_fetch"):
    with st.spinner(f"Interrogation de l'API NVD pour {cve_selectionnee}..."):
        ...
        details = fetch_cve_details(cve_selectionnee, lang=lang)
        time.sleep(0.5)  # Respecter la limite de l'API NVD
