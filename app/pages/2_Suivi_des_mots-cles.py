# 2_kpi2_Mots_cles.py -- Version Sentinel Rain (Code Corrigé)
...
.page-title { text-align:center; font-size:2.8rem; font-weight:700; color:#a855f7; margin-bottom:20px; }
...
.article-link:hover { color: #a855f7; }
</style>
...
(function() {
  ...
  function startCodeRain(){
    ...
    cv.style.cssText='position:fixed;top:0;left:0;width:100vw;height:100vh;z-index:0;pointer-events:none;opacity:0.08;';
    ...
    function draw(){
      ...
      ctx.fillStyle = '#a855f7'; ctx.font = fontSize + 'px Roboto Mono';
...
# ── MÉTRIQUES AVEC COMPTEUR ET SURBRILLANCE ──────────────────────────────────
...
.btn {{ 
    ...
    border-radius: 4px; padding: 10px; cursor: pointer; font-size: 0.8rem; width: 100%;
}}
</style>
<div class="grid">
  <div class="card"><div class="label">Mots-clés Actifs</div><div class="value" id="count-k2">0</div></div>
  <div class="card"><div class="label">Top Volume</div><div class="value purple">{top_v}</div></div>
  <div class="card"><div class="label">Top Vélocité</div><div class="value green">{top_a}</div></div>
...
if selected_kw != "-- Choisir --":
    ...
    for _, row in relevant.iterrows():
        st.markdown(f"""<div class="article-card"><a href="{row['url']}" target="_blank" class="article-link">{row['title']}</a><br><small>{row['source']} | {str(row['published_date'])[:10]}</small></div>""", unsafe_allow_html=True)