...
.page-title{text-align:center;font-size:2.8rem;font-weight:700;color:#3b82f6;margin-top:10px;margin-bottom:10px;text-shadow: 0 0 15px rgba(59,130,246,0.3);}
.filter-container {
...
/* Styling du selectbox pour qu'il s'intègre au thème sombre */
...
# Conteneur pour le filtre de temps
with st.container():
    ...
    with col_f2:
        ...
        # Mapping du choix vers le nombre de jours
        days_map = {
            "3 derniers jours": 3,
            "7 derniers jours": 7,
...
# Données filtrées pour le graphique d'évolution
df_filtered = df[df['published_date'] >= limit_date]

# Calcul vélocité (comparaison 24h vs historique de la période choisie)
...
# On calcule la moyenne journalière sur la période moins le jour actuel
...
if not trend_df.empty:
    ...
    if top_threat['velocity_score'] > 2.0:
        ...
        <div class="alert-banner">
            ...
            présente une accélération notable ({top_threat['velocity_score']:.1f}x la normale sur {selected_days}j).
        </div>
        """, unsafe_allow_html=True)
...
# On prend les sources les plus actives pour la clarté du graphique
